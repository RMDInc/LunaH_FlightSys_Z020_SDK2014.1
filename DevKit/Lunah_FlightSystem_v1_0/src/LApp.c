...
/*	Xil_Out32 (XPAR_AXI_GPIO_4_BASEADDR, 12);
	...
	while(1){
		...
		xil_printf(" 10) Execute Print of Data on DRAM \n\r");
		xil_printf("******************************\n\n\r");
		...
		if ( menusel < 0 || menusel > 10 ) {
			xil_printf(" Invalid Command: Enter 0-10 \n\r");
...
//////////////////////////// DAQ ////////////////////////////////
//////////////////////////// control of power regulators ////////////////////////////////
void PWRCONTROL(){
	int power;

	xil_printf("\n\r Select Operation \n\r");
	xil_printf("******************************\n\r");
	xil_printf(" 0) Enable Sensor Head\n\r");
	xil_printf(" 1) Enable data acquisition\n\r");
	xil_printf(" 2) Enable TEC controller Data\n\r");
	xil_printf(" 3) Enable TEC bleed mode\n\r");
	xil_printf("******************************\n\n\r");

	ReadCommandPoll();
	sscanf(RecvBuffer,"%01d",&power);

	switch(power){
	case 0:
		xil_printf("\n\r Select 0 to disable or 1 to enable sensor head \n\r");
		ReadCommandPoll();
		sscanf(RecvBuffer,"%01d",&power);
		if ( power < 0 || power > 1 ) {
			xil_printf(" Invalid Command: Enter 0-1 \n\r");
			sleep(1); 			// Built in Latency ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 1 s
		}
		Xil_Out32 (XPAR_AXI_GPIO_7_BASEADDR, power);
		break;
	case 1:
		xil_printf("\n\r Select 0 to disable or 1 to enable data acquisition \n\r");
		ReadCommandPoll();
		sscanf(RecvBuffer,"%01d",&power);
		if ( power < 0 || power > 1 ) {
			xil_printf(" Invalid Command: Enter 0-1 \n\r");
		...
		Xil_Out32 (XPAR_AXI_GPIO_6_BASEADDR, power);
		break;
	case 2:
		xil_printf("\n\r Select 0 to disable or 1 to enable TEC controller \n\r");
		ReadCommandPoll();
		sscanf(RecvBuffer,"%01d",&power);
		if ( power < 0 || power > 1 ) {
			xil_printf(" Invalid Command: Enter 0-1 \n\r");
		...
		Xil_Out32 (XPAR_AXI_GPIO_4_BASEADDR, power);
		break;
	case 3:
		xil_printf("\n\r Select 0 to disable or 1 to enable TEC bleed mode \n\r");
		ReadCommandPoll();
		sscanf(RecvBuffer,"%01d",&power);
		if ( power < 0 || power > 1 ) {
			xil_printf(" Invalid Command: Enter 0-1 \n\r");
		...
		Xil_Out32 (XPAR_AXI_GPIO_5_BASEADDR, power);
		break;
